			...
			validate: (input) => {
				try {
					...
					return (
						...
						` Make sure the JSON is minified or encoded, and and does not contain line breaks.`
							.red
					);
				}
			},
	...
	return inquirer.prompt([
		{
			...
			choices: boilerplates.map((b) => ({
				name: `${b.name} ${colors.gray(`(${b.repo})`)}`,
				value: b.alias,
			})),
...
export async function askActionWithExistingFolder() {
	return inquirer.prompt({
		...
		choices: [
			...
			{
				name: `Continue with this folder ${colors.gray(
					`(git clone will be skipped)`
				)}`,
				short: 'Continue with folder',
				value: 'continue',
			},
			{
				name: `Override existing folder ${colors.gray(
...
export async function askToProceedWithProvidedData(hasData = true) {
	return inquirer.prompt({
		...
		message: hasData
			? `Do you want to continue with the provided data?`
			: `Do you want to continue without data?`,
		default: true,
	});
}
